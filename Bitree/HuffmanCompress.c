...
// 统计字符频度的临时结点
typedef struct {
	unsigned char eightBits;			// 以8bits为单元的无符号字符
...
// 生成哈夫曼编码
void HufCode(HTNode *HT, unsigned char_kinds)
{
	unsigned int i;
	int cur, next, index;
	...
	for (i = 0; i < char_kinds; ++i)
	{
		index = 256 - 1;	// 编码临时空间索引初始化
							...
							// 从叶子向根反向遍历求编码
		for (cur = i, next = HT[i].parent; next != 0;
			cur = next, next = HT[next].parent)
		...
		strcpy(HT[i].code, &code_tmp[index]);     // 正向保存编码到树结点相应域中
	}
...
int compress(char *ifname, char *ofname)
{
	...
	*/
	...
	for (i = 0; i < 256; ++i)
	{
		...
		tmp_nodes[i].eightBits
 = (unsigned char)i;		// 数组的256个下标与256种字符对应
	}
	...
	if (char_kinds == 1)
	{
		...
		fwrite((char *)&tmp_nodes[0].eightBits
, sizeof(unsigned char), 1, outfile);		// 写入唯一的字符
	...
	else
	{
		...
		HT = (HTNode *)malloc(node_num * sizeof(HTNode));		// 动态建立哈夫曼树所需结点     
	 ...
	 = tmp_nodes[i].eightBits
		...
		CreateTree(HT, char_kinds, node_num);		// 创建哈夫曼树

		HufCode(HT, char_kinds);		// 生成哈夫曼编码
		...
		for (i = 0; i < char_kinds; ++i)
		{
			fwrite((char *)&HT[i].eightBits
	, sizeof(unsigned char), 1, outfile);			// 写入字符（已排序，读出后顺序不变）
...
int extract(char *ifname, char *ofname)
{
	...
	else
	{
		...
		for (i = 0; i < char_kinds; ++i)
		{
			fread((char *)&HT[i].eightBits
	, sizeof(unsigned char), 1, infile);		// 读入字符
